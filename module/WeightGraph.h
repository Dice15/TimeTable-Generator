...
private:	// 역방향 그래프에 대해 최단경로를 구함
	void Dijkstra_Of_ReverseGraph(vector<int>& usedEdge, vector<int>& visitedVertex)
	{
		...
		mDistance.assign(mGraphSize, mMaxValue);
...
public:		// 추가로 Ksp를 진행하고 그 결과를 반환한다
	vector<Path> Additional_Ksp(int kValue, vector<Bits>& vertexConflict, function<int(int)>& GetVertexNum)
	{
		...
		while (!mKspPq.empty() && (int)mPath.size() < kValue) {
			...
			if (newPxBack == node->curr_vertex) InsertVertexToPrefix(newPrefix, newPxBack, node->next_vertex);
			else {
				for (int vertex = mNextVertex[newPxBack]; vertex != node->curr_vertex; vertex = mNextVertex[vertex]) InsertVertexToPrefix(newPrefix, newPxBack, vertex);
				InsertVertexToPrefix(newPrefix, newPxBack, node->curr_vertex);
				InsertVertexToPrefix(newPrefix, newPxBack, node->next_vertex);
			}

			AddPath(newPrefix, mSuffixPath[mNextVertex[newPxBack]]);

			for (int i = 0; i < 2; i++) {
				if (node->son[i] != (PersistentHeap::Node*)nullptr)
