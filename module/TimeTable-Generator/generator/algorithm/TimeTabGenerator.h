...
private:
	...
	pair<int, int> mTargetCredit;                    // 목표학점
...
public:
	TimeTabGenerator() {}
	TimeTabGenerator(int kValue, pair<int, int>& targetCredit, int numberOfEssentialCourse, int numberOfNoramlCourse, vector<pair<Course, int>>& courseInfoList)
		: mValidInput(true)
...
private:   // KspGraph에 필수과목 추가
	void Add_EssentialCourse()
	{
		...
		for (auto& [id_base, idxList] : classify) {   // Source, 필수과목 그래프 연결
			...
			prevIdxList = idxList;
			mTargetCredit.first -= mCourseInfoList[idxList[0]].first.credit;
			mTargetCredit.second -= mCourseInfoList[idxList[0]].first.credit;
		}


		// 목표학점에 따른 예외 처리
		if (mTargetCredit.second < 0) {   // 목표학점을 초과하는 경우
			mValidInput = false;
		}
		else {
			mSink = mSubSource + (mNumberOfNormalCourse * mTargetCredit.second) + 1;
...
private:   // KspGraph에 일반과목 추가
	...
	{
		if (mTargetCredit.second < 0) return;
	
		auto& [fstLevel, sndLevel] = mTargetCredit;
		...
		for (int v = mNumberOfEssentialCourse + 1; v < mNumberOfCourse; v++) {   // TODO: Source로부터의 간선 최적화 필요 
			if (mCourseInfoList[v].first.credit > sndLevel) continue;
			mKspGraph.Add_Directed_Edge(mSubSource, convVertexNum(v, mCourseInfoList[v].first.credit), Get_CourseWeight(v));
		}


		// Sink와 일반과목 연결
		for (int lev = max(0, fstLevel); lev <= sndLevel; lev++) {
			if (lev == 0) mKspGraph.Add_Directed_Edge(mSubSource, mSink, 0);
			else {
				for (int u = mNumberOfEssentialCourse + 1; u < mNumberOfCourse; u++)   // TODO: Sink로의 간선 최적화 필요 
					mKspGraph.Add_Directed_Edge(convVertexNum(u, lev), mSink, 0);
			}
		}


		// 일반 과목끼리 연결
		for (int lev = 1; lev < sndLevel; lev++) {   // TODO: 간선 최적화 필요
			for (int u = mNumberOfEssentialCourse + 1; u < mNumberOfCourse; u++) {
				for (int v = u + 1; v < mNumberOfCourse; v++) {
					...
					if (lev + mCourseInfoList[v].first.credit > sndLevel) continue;
...
public:   // Ksp를 추가로 실행하여 시간표를 생성하고 반환
	tuple<vector<vector<Course>>, vector<vector<int>>, GetTimeTabResult> Get_TimeTable(int page)
	{
		...
		vector<vector<Course>> courseTimeTab;     // 포함된 과목 (시간표별)
		vector<vector<int>> lankTimeTab;          // 우선순위 카운팅 (시간표별)


		for (int i = begin; i < end; i++) {
			courseTimeTab.push_back(vector<Course>());
			lankTimeTab.push_back(vector<int>(6, 0));

			auto& courseList = courseTimeTab.back();
			auto& lankList = lankTimeTab.back();

			for (int j = 1; j < mNumberOfCourse; j++)
				if (mTimeTabList[i] & (Bits)1 << (Bits)j) {
					courseList.push_back(mCourseInfoList[j].first);
					lankList[mCourseInfoList[j].second]++;
				}
		}

		return { courseTimeTab, lankTimeTab, GetTimeTabResult::LoadSuccessfully };
	}
};