...
namespace TimeTableGenerator {
	...
	public:
		...
		TimeTabForm(pair<int, int>& targetCredit, int numberOfEssentialCourse, int numberOfNoramlCourse, vector<pair<Course, int>>& courseInfoList)
		{
			isInitializing = true;
			InitializeComponent();
...
#pragma endregion
	...
	private:
		System::Void InitialzerMembers(pair<int, int>& targetCredit, int numberOfEssentialCourse, int numberOfNoramlCourse, vector<pair<Course, int>>& courseInfoList) {
	...
	private:	// 시간표 페이지 관리
		...
		System::Void LoadPage(PageMove pageMove) {
			...
			auto [courseTimeTab, lankTimeTab, resultMessage] = mGenerator->Get_TimeTable(mPageIndex);
			int timeTabCnt = (int)courseTimeTab.size();
			...
			case TimeTabGenerator::LoadSuccessfully:
				...
				for (int i = 0; i < kValue; i++) {
					...
					if (i < timeTabCnt) {
						...
						for (auto& course : courseTimeTab[i]) {
							credit += course.credit;
							mCourseOfTimeTab->back().push_back(course);
							AddCourse(mTimeTabList[i], course);
						}

						mColumnList[i]->Text = "시간표" + (kValue * (mPageIndex - 1) + i + 1) + " (" + credit + "학점)";
						CountCourseLank(mCourseLankCountL[i], mCourseLankCountR[i], lankTimeTab[i]);
					}
				}
